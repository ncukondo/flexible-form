const uuid = (() => {
  const HEX_OCTETS = Object.freeze(
    [...Array(0x100)].map((e, i) => i.toString(0x10).padStart(2, "0").toUpperCase()),
  );
  const VERSION = 0x40;
  const VARIANT = 0x80;
...
const remap = (source: readonly number[], { from, to }: { from: number; to: number }) => {
  ...
  const split = (n: number, i: number): readonly [l: number[], m: number[], r: number[]] => {
...
const sliceByLen = <T extends ReadonlyArray<unknown> | string>(src: T, len: number) => {
  const size = Math.ceil(src.length / len);
  return [...Array(size).keys()].map(i => src.slice(i * len, i * len + len)) as T extends string
    ? string[]
    : T[];
};

const toShortUUID = (uuid: string) => {
  const bytes = sliceByLen(uuid.replaceAll("-", ""), 2).map(t => parseInt(t, 16));
